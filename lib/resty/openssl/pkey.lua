...
function _M.new(s, opts)
  ...
  -- OpenSSL 3.0 seems always return 0 when key is generated by EVP_PKEY_keygen
  -- and EVP_PKEY_new_raw_*_key
  if OPENSSL_30 and type(ctx) == "table" then
    key_type = ctx[2]
    ctx = ctx[1]
  end
  ...
  key_type = key_type or C.EVP_PKEY_base_id(ctx)
  if key_type == 0 then
