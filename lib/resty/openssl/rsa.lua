...
local function cleanup_with_error(cleanup_from_idx, opts_bn, err)
...
function _M.set_parameters(rsa_st, opts)
    ...
    for k, v in pairs(opts) do
        ...
        if err then
            ...
            return cleanup_with_error(cleanup_from_idx, opts_bn)
        end
    ...
    if OPENSSL_11_OR_LATER then
        ...
        if do_set_key then
            ...
            if code == 0 then
                ...
                return cleanup_with_error(cleanup_from_idx, opts_bn, err)
            end
        end
