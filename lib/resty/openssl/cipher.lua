...
function _M.new(typ)
    ...
    if dtyp == nil then
        return nil, ("cipher.new: invalid cipher type \"%s\""):format(typ)
    end
...
function _M.init(self, key, iv, opts)
    opts = opts or {}
    if not key or #key ~= self.key_size then
        return false, ("cipher:init: incorrect key size, expect %d"):format(self.key_size)
    end
    if not iv or #iv ~= self.iv_size then
        return false, ("cipher:init: incorrect iv size, expect %d"):format(self.iv_size)
    end
...
function _M.encrypt(self, key, iv, s, no_padding, aead_aad)
    ...
    if aead_aad then
        _, err = self:update_aead_aad(aead_aad)
        if err then
            return nil, err
        end
    end
...
function _M.decrypt(self, key, iv, s, no_padding, aead_aad, aead_tag)
    local _, err = self:init(key, iv, {
        is_encrypt = false,
        no_padding = no_padding,
    })
    ...
    if aead_tag then
        _, err = self:set_aead_tag(aead_tag)
        if err then
            return nil, err
        end
    end
...
function _M.update_aead_aad(self, aad)
...
function _M.get_aead_tag(self, size)
    if not self.initialized then
    ...
    if size > self.key_size then
        return nil, ("tag size %d is too large"):format(size)
    end
...
function _M.set_aead_tag(self, tag)
    if not self.initialized then
...
function _M.derive(self, key, salt, count, md)
    ...
    if salt then
        if #salt > 8 then
            ngx_log(ngx.WARN, "cipher:derive: salt is too long, truncate salt to 8 bytes")
            salt = salt:sub(0, 8)
        elseif #salt < 8 then
            ngx_log(ngx.WARN, "cipher:derive: salt is too short, padding with zero bytes to length")
            salt = salt .. (('\000'):rep(8 - #salt))
        end
    end

    local mdt = C.EVP_get_digestbyname(md or 'sha1')
    if mdt == nil then
        return nil, nil, ("cipher:derive: invalid digest type \"%s\""):format(md)
    end
    local cipt = C.EVP_CIPHER_CTX_cipher(self.ctx)
    local keyb = uchar_array(self.key_size)
    local ivb = uchar_array(self.iv_size)

    local size = C.EVP_BytesToKey(cipt, mdt, salt, key, #key, count or 1, keyb, ivb)
    if size == 0 then
