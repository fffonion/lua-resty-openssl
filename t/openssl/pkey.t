...
=== TEST: pkey: loads a provider-based key (ML-DSA-65) and resolves its key type
On OpenSSL < 3.5 (no ML-DSA) the test is skipped by emitting the expected output.
--- http_config eval: $::HttpConfig
--- config
    location =/t {
        content_by_lua_block {
            if require("resty.openssl.version").version_num < 0x30500000 then
                ngx.say("id-ml-dsa-65")
                ngx.exit(0)
            end

            local pkey = require("resty.openssl.pkey")
            -- provider-based keys have no legacy base id; loading previously
            -- failed with "cannot get key_type".
            local p = myassert(pkey.new(require("mldsa65_key")))
            ngx.say(p:get_key_type().sn)
        }
    }
--- request
    GET /t
...
=== TEST: pkey: generates a provider-based key (ML-DSA-65)
On OpenSSL < 3.5 (no ML-DSA) the test is skipped by emitting the expected output.
--- http_config eval: $::HttpConfig
--- config
    location =/t {
        content_by_lua_block {
            ...
            local pkey = require("resty.openssl.pkey")
            -- provider-based keys are generated by name (no legacy key type id)
            local p = myassert(pkey.new({ type = "ML-DSA-65" }))
            ngx.say(p:get_key_type().sn)
            ngx.say(tostring(p:to_PEM("private") ~= nil))
        }
    }
--- request
    GET /t
